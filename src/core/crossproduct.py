...
class SignLanguageCP(CrossProduct[np.ndarray, np.ndarray, np.ndarray, None]):
    """Cross product of the Sign Language environment."""
    ...
    def _get_obs(
        ...
        u_enc[u] = 1
        crm_cfg = u_enc
        return np.concatenate((ground_obs, crm_cfg), axis=0)
